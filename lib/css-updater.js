 ...
 * @param {Array.<string>} fixSelectors 修复的选择器
 */
function updateRuleStyle(rule, filePath, spriteSheets, options, fixSelectors) {
    var added;
    ...
    spriteDeclares.forEach(function (item) {
        ...
        declare.value = declare.value.replace(CSS_URL_REGEXP, function (match, url) {
            var spriteSheet = item.spriteSheet;

            if (!added) {
                added = true;
                options.fixIE6PNG && spriteSheet.dpr === 1
                && fixSelectors.push.apply(fixSelectors, rule.selectors);
            }
            return match.replace(url, util.getReferencePath(spriteSheet.path, filePath));
        });
    ...
    (rule.rules || []).forEach(function (r) {
        updateRuleStyle(r, filePath, spriteSheets, options, fixSelectors);
    });
}
 ...
 * @param {Object} options 选项
 * @return {Object} 返回要修复的 ie6 选择器 map, key 为修复的样式文件路径，value 为修改的样式规则选择器列表
 */
exports.update = function (spriteSheets, styleFiles, options) {
    var updateStyleSelectorMap = {};
    styleFiles.forEach(function (file) {
        ...
        try {
            ...
            rules.forEach(function (rule) {
                updateRuleStyle(rule, filePath, spriteSheets, options, fixSelectors);
            });

            file.data = css.stringify(ast);
            updateStyleSelectorMap[filePath] = fixSelectors;
        }
        catch (e) {
